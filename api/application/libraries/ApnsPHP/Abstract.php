...
abstract class ApnsPHP_Abstract
{
	...
	public function connect()
	{
		...
		while (!$bConnected) {
			try {
                            $this->_start_proc=$before = microtime(true);
				$bConnected = $this->_connect();
                                $this->_log('<br>Connect time: ' . (microtime(true)-$before));
	...
	public function disconnect()
	{
		if (is_resource($this->_hSocket)) {
                        $this->_end_proc = microtime(true);
                        $this->_log('Proccess time: '.($this->_end_proc - $this->_start_proc));
			$this->_log('INFO: Disconnected.<br>');
