	...
	public function log($sMessage)
	{
		/*printf("%s ApnsPHP[%d]: %s\n",
			date('r'), getmypid(), trim($sMessage)
                ...
                $name = 'tmp/'.date('Ymd').'_push_log.txt';
                $handle = @fopen($name, "a");
				if(!$handle){
					$path =  __FILE__;
                    $path = str_replace('api/application/libraries/ApnsPHP/Log/Embedded.php','',$path);
                    $handle = fopen($path . $name, "a");
				}
                fwrite($handle, "%s ApnsPHP[%d]: %s".date('r').getmypid().trim($sMessage));
                fwrite($handle, "\r\n");
                fclose($handle);
	}
}
